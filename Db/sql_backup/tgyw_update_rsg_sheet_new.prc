create or replace procedure smzj.tgyw_update_rsg_sheet_new(p_sheet_id in varchar2, --订单编号
...
---- *************************************************************************
-- SUBSYS:     托管服务
-- PROGRAM:      tgyw_update_rsg_sheet_new
-- RELATED TAB:
-- SUBPROG:
-- REFERENCE:
-- INPUT:
...
--               -1   数据库错误
-- AUTHOR:       卢雅琴
-- DESCRIPTION:  修改认/申购订单【订单表 简易录单版】
-- CREATE DATE:  2016-04-21
-- VERSION:
-- EDIT HISTORY:
-- *************************************************************************
 v_job_name varchar2(100) :='tgyw_update_rsg_sheet_new';
...
begin
    ...
    if v_son_buy_monther='true' then
        ...
        v_flag := '1';
        v_msg := '投资人申购子基金出错';
        insert into t_fund_job_running_log (JOB_NAME,job_running_log) values ('子买母问题',v_son_buy_monther || '    '||p_credit_id || '    '|| p_pt_id || v_current_date );
        open c_result for select 'faile' as res, v_msg as msg  from dual;
        commit;
        return;
    end  if;
    ...
    --订单是否已申请过开始
    select count(1) into v_buy_count from busi_sheet t where t.credit_id=p_credit_id and t.pt_id=p_pt_id  and to_char(t.sheet_create_time,'yyyy-MM-dd') = p_apply_date  and t.is_delete='0' and t.id!=p_sheet_id;
    ...
    if v_is_limit_buy=1 then
     ...
     if v_flag <> 1 then
       update busi_sheet set update_date = v_current_date,pt_id=p_pt_id,pt_no=v_pt_no,apply_amount=p_amount,credit_id=p_credit_id,DT_ID=p_detail_id,REMARK=p_sheet_remark where id = p_sheet_id;
