create or replace procedure smzj.TGYW_PRODUCT_SYN_QUAR_H_REPORT(
...
-- PROGRAM:      TGYW_PRODUCT_SYN_QUAR_H_REPORT
...
type MyRefCurA IS  REF CURSOR RETURN BUSI_QUARTERLY_REPORT%RowType;

vRefCurA  MyRefCurA;

vTempA  vRefCurA%RowType;

v_job_name       varchar2(100):='TGYW_PRODUCT_SYN_QUAR_H_REPORT';
...
begin
  ...
  loop
      ...
      fetch vRefCurA into vTempA;
       ...
       update BUSI_QUARTERLY_REPORT set HISTORY_STATUS='1',UPDATE_DATE=v_today,UPDATE_BY=p_mgrname  where  id=vTempA.id;
...
end TGYW_PRODUCT_SYN_QUAR_H_REPORT;
/
