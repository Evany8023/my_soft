create or replace procedure smzj.tgyw_redeem_sheet(p_company_id in varchar2, --公司ID
...
-- *************************************************************************
  ...
  cursor data_cur  is select t.investor_name,t.credit_no,t.credit_type_no,t.credit_type,t.id,t.investor_type_no,t.product_no,t.company_no,t.redeem_share
    from busi_batch_redeem_tmp t
   where t.STATUS = '0'
     and t.COMPANY_ID = p_company_id
     and t.PRODUCT_ID = p_product_id
     and t.APPLY_DATE = to_date(p_applay_date, 'yyyy-MM-dd');
...
begin
    ...
    select distinct CREDIT_NO,CREDIT_TYPE_NO  from  busi_batch_redeem_tmp t
     where   t.COMPANY_ID = p_company_id    and t.PRODUCT_ID = p_product_id  and to_char(t.APPLY_DATE, 'yyyy-MM-dd') = p_applay_date and t.status = '0' );
   ...
   select to_date(p_applay_date ||v_current_time,'yyyy-MM-ddHH24:MI:SS'  ) into v_current_date from dual;
    ...
    open data_cur ;
    loop
        fetch data_cur into v_tzrxm,v_zjhm,v_zjlbcode,v_zjlb,v_id,v_tzrlx,v_fundcode,v_comcode,v_shfe;
        ...
        if v_kh_count >0 then
           ...
           if v_son_buy_monther='true' then
                 v_flag := '1';
                v_msg := v_msg||'投资人(''' ||v_zjhm ||''')申购子基金，会自动出发子基金申购母基金，请勿重复录单<br>';
             ...
             select count(id) into v_count from busi_sheet where  credit_id= v_credit_id and  pt_id = p_product_id  and to_char(sheet_create_time,'yyyy-MM-dd')=p_applay_date and BUSINESS_TYPE='024' and is_delete='0';
            if v_count> 0 and v_flag = 0  then
                v_flag :=1;
                v_msg := v_msg||'此客户，证件号码('|| v_zjhm||')  已在申请日期:'||p_applay_date||' 提交过该基金的赎回申请' ||'<br>';
           end if;
    ...
    if v_flag = 1 then
        ...
        update busi_batch_redeem_tmp t set status='1' where t.company_id=p_company_id and t.product_id=p_product_id  and to_char(t.APPLY_DATE, 'yyyy-MM-dd') = p_applay_date;
        v_msg :='生成数据成功';
