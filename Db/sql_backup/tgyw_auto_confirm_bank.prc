create or replace procedure smzj.TGYW_AUTO_CONFIRM_BANK  is

----
-- SUBSYS       托管服务
-- PROGRAM      TGYW_AUTO_CONFIRM_BANK
-- RELATED TAB
-- SUBPROG
-- REFERENCE
-- AUTHOR       谷旭
-- DESCRIPTION  自动确认银行账号
...
v_job_name                    varchar2(100):= '自动确认银行账号';
...
--自动确认的范围为：超过指定时间的非确认的唯一匹配的记录
cursor bank_cur  is select t.apply_no,t.credit_id,t.product_id,t.company_id,t.account_no,t.id  from busi_bind_bank_card_match t inner join busi_investor_credit c on c.id=t.credit_id where  t.confirm_status='0' and t.match_status='1' and t.create_date+2<sysdate
and ((length(t.account_no)>=16 and length(t.account_no)<=19) or c.user_type='2') and t.bank_no is not null;
