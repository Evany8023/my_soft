CREATE OR REPLACE PROCEDURE SMZJ.tgyw_delete_rgsg_trade_data (p_fundid in  varchar2, p_compid in varchar2, todaydate in varchar2,flag out varchar2,msg out varchar2 ) is
---- *************************************************************************
-- SUBSYS:     托管服务
...
-- *************************************************************************
  ...
  v_job_name varchar2(100) :='tgyw_pt_delete_trade_data';
  ...
  v_credit_account varchar2(50 char );
  v_count integer :=0;
...
cursor sale_data_cur  is select t.credit_no,t.credit_type_code,t.bank_accunt_no
...
begin
  ...
  open sale_data_cur ;
   loop
      fetch sale_data_cur into v_zjhm ,v_zjlb,v_credit_account;
       if sale_data_cur%notfound then
        ...
        if v_jy_count= 1   and v_count_jy=1 then
         ...
         begin
          select v.id  into  v_credit_bank_id from busi_bind_bank_card v where  v.credit_id=v_credit_id and v.product_id = p_fundid  and v.bind_date > v_yestoday;
        ...
        if v_credit_bank_id is not null then
            select count(*)  into v_count_jy from ( select count(1) from  Busi_Sheet t where t.credit_id=v_credit_id  and   t.business_type in('020','022') and t.pt_id=p_fundid and t.bank_card_id=v_credit_bank_id  and t.is_delete='0'   group by  id  );
            if v_count_jy<2 then
               delete  from  busi_bind_bank_card  t where t.credit_id=v_credit_id and t.product_id = p_fundid  and t.company_id=p_compid  and  to_char(t.bind_date,'yyyy-mm-dd')=todaydate ;
            end if;
         end if;
...
end tgyw_delete_rgsg_trade_data;
/
