CREATE OR REPLACE PROCEDURE SMZJ.F_DELETE_RSG (
...
begin
     v_flag:=0;
    if trim(p_sheet_id) is null  or  trim(p_cp_id) is null then
        v_flag := 1;
        v_msg  := '订单ID,公司ID必填';
        raise ve_exception;
    end if;
    ...
    -- 当天开户
        ...
        --删除和公司关联关系，本公司只有一个本产品关联，可以删除
         --删除赎回临时表的相应订单
          delete from BUSI_BATCH_REDEEM_TMP d where exists (select 1
          from busi_sheet s
         inner join busi_investor_credit c
            on s.credit_id = c.id
         where s.id = p_sheet_id
           and s.company_id = p_cp_id
           and s.pt_id = d.product_id
           and c.credit_type = d.CREDIT_TYPE_NO
           and c.credit_no = d.credit_no
           and to_char(s.SHEET_CREATE_TIME, 'yyyyMMdd') =
               to_char(d.apply_date, 'yyyyMMdd')
           and d.status = 1
           and s.business_type='024');
          --删除临时表的相应订单(认申购)
          delete from busi_sale_data d 
          where exists (select 1 from busi_sheet s inner join busi_investor_credit c on s.credit_id=c.id 
          where s.id=p_sheet_id 
          and s.company_id=p_cp_id
          and s.pt_id=d.product_id 
          and c.credit_type=d.credit_type_code 
          and c.credit_no=d.credit_no 
          and to_char(s.SHEET_CREATE_TIME,'yyyyMMdd')=to_char(d.apply_date,'yyyyMMdd')
          and d.status=1 
          and s.business_type in ('020','022'));
        --仅仅买了一个产品，且产品是本产品
        ...
        select count(*)  into v_count_jy from ( select count(1) from  Busi_Sheet t where t.credit_id=v_credit_id and   t.business_type in('020','022') and t.pt_id=v_pt_id   and t.is_delete='0'   group by  id  );
        if v_count_jy<2 then
          delete  from  busi_bind_bank_card  t where t.credit_id=v_credit_id and t.product_id = v_pt_id  and t.company_id=p_cp_id  and  t.bind_date>v_yestoday ;
        end if;
        --删除 开户祥情的 带上sheet_no 确认是本订单创建的才可以删除
         select count(*)  into v_count_jy from ( select count(1) from  Busi_Sheet t where t.credit_id=v_credit_id and t.company_id= p_cp_id  and   t.business_type in('020','022') and t.is_delete='0'   group by  id  );
        ...
        if  v_count_jy <2 and v_kh_count2 <2 then
          ...
          delete  from  busi_sheet t where id=p_sheet_id and t.company_id=p_cp_id;
    end if;
