create or replace procedure smzj.tgyw_rsg_sheet(
                                                      p_company_id in varchar2,
                                                      p_product_id in varchar2,
                                                      p_applay_date   in varchar2,
...
-- *************************************************************************
 ...
 v_buy_count number; --购买次数
  ...
  cursor data_cur  is select t.investor_name,t.credit_no,t.credit_type_code,t.legal_person,t.handel_person,t.bank_no,t.bank_name,t.open_bank_name,t.bank_accunt_no,t.amount,t.province_name,t.city_name,t.investor_type,t.bank_account_name,t.id
    from busi_sale_data t
   where t.status = 0
     and t.company_id = p_company_id
     and t.product_id = p_product_id
    and to_char(t.APPLY_DATE, 'yyyy-MM-dd') = p_applay_date;
  cursor data_cur_check  is select t.investor_name,t.credit_no,t.credit_type_code
    from busi_sale_data t
   where t.status = 0
     and t.company_id = p_company_id
     and t.product_id = p_product_id
...
begin
  v_flag:=0;
    select count(*) into v_count from busi_sale_data t  where t.status = 1  and t.company_id = p_company_id  and t.product_id = p_product_id and  to_char(t.APPLY_DATE, 'yyyy-MM-dd') = p_applay_date;
    if v_count > 0 then
    ...
    select count(*) into v_hasimportdata from  busi_sale_data t  where t.status = 0  and t.company_id = p_company_id   and t.product_id = p_product_id and to_char(t.APPLY_DATE, 'yyyy-MM-dd') = p_applay_date;
   ...
   select to_date(p_applay_date ||to_char(sysdate,'HH24:MI:SS' ),'yyyy-MM-ddHH24:MI:SS'  ) into v_current_date from dual;
 ...
 open data_cur_check ;
    ...
    if v_flag = 1 then
     ...
     v_sh_je_ta:=null;
     v_rsh_je_ta:=null;
    select count(1) into v_limit_count  from zsta.fundlimit@TA_dblink t, busi_product info where info.product_no=t.fundcode and info.id=p_product_id;             
    if v_limit_count > 0 then
                  select minsubsamountbyindi, minbidsamountbyindi,minappbidsamountbyindi into v_rg_je_ta,v_sh_je_ta,v_rsh_je_ta from zsta.fundlimit@TA_dblink t, busi_product info where info.product_no=t.fundcode and info.id=p_product_id;  
    end if;
    ...
    open data_cur ;
    loop
        fetch data_cur into v_investor_name,v_credit_no,v_credit_type_code,v_legal_person,v_handel_person,v_bank_no,v_bank_name,v_open_bank_name,v_bank_accunt_no,v_rgje,v_province_name,v_city_name,v_investor_type,v_bank_account_name,v_tab_id;
        if data_cur%notfound then
          exit;
        end if;
        
        select decode(v_investor_type,'机构','2','个人','1') into v_user_type from dual;           
        select count(*) into v_kh_count from busi_investor_credit t where t.credit_type = v_credit_type_code and t.credit_no = v_credit_no ;
        --如果是新开户，则作三张表插入操作
        if v_kh_count=0 then
              ...
              insert into busi_investor_detail(id,investor_type,handle_person,legal_person,name,create_by,busi_type,company_id,apply_no,create_date,credit_type,credit_id,credit_no,manage_fee_mark,achieve_fee_mark,pt_id)
                 values(v_khsqid,v_user_type,v_handel_person,v_legal_person,v_investor_name,p_create_by,'001',p_company_id,v_apply_no,v_current_date,v_credit_type_code,v_credit_id,v_credit_no,'是','是',p_product_id);
                 --关联基金公司
        ...
        if v_kh_count >0 then
            ...
            if v_khsq_count =0 then
                ...
                insert into busi_investor_detail(id,investor_type,handle_person,legal_person,name,create_by,busi_type,company_id,apply_no,create_date,credit_type,credit_id,credit_no,manage_fee_mark,achieve_fee_mark,pt_id)
                   values(v_khsqid,v_user_type,v_handel_person,v_legal_person,v_investor_name,p_create_by,(case when v_khsq_count>0 then '008' else '001' end),p_company_id,v_apply_no,v_current_date,v_credit_type_code,v_credit_id,v_credit_no,'是','是',p_product_id);
        ...
        select child_buy_or_sale_prarent(v_credit_id,p_product_id,'01',null) into v_son_buy_monther from dual;
       ...
       if v_son_buy_monther='true' then
          ...
          insert into t_fund_job_running_log (JOB_NAME,job_running_log) values ('子买母问题',v_son_buy_monther || '    '||v_credit_id || '    '|| p_product_id || v_current_date );
       end  if;
       
        select count(1) into v_buy_count from busi_sheet t where t.credit_id=v_credit_id and t.pt_id=p_product_id and t.business_type in('020','022')  and to_char(t.sheet_create_time,'yyyy-MM-dd')  = p_applay_date  and t.is_delete='0' ;
            if v_buy_count> 0 then
               v_flag:=1;
               v_msg := v_msg||'证件号码：'||v_credit_no||',姓名：'||v_investor_name||',已经在【'||p_applay_date|| '】认购、申购过此产品,不需要重新录入;'||';<br>';
            end if; 
            
        --查询基金状态
        select t.publish_status,t.is_limit_buy,start_price,t.product_no,name into  v_publish_status,v_is_limit_buy,v_start_price,v_product_no,v_product_name from busi_product t  where t.id=p_product_id;
        --本地是发行状态，则类型为认购，本地不是发行状态则根据产品代码与日期去TA查询基金状态，非发行则全为申购
        if (v_publish_status='1' or v_publish_status='2')       and v_tody_time = p_applay_date  then
              v_busi_type :='认购';
              v_busi_type_code :='020';                  
        elsif ( v_publish_status='0'  or v_publish_status='6')  and v_tody_time = p_applay_date   then  -- 0 正常交易，6 （停止赎回）可申购
            v_busi_type :='申购';
           ...
           -- select t.jjzt into v_publish_status from zsta.uv_xx_fundstatus@ta_dblink t where t.jjdm =v_product_no and t.rq=to_date(p_applay_date,'yyyy-MM-dd');
            begin 
                select t.jjzt,1 into v_publish_status,v_pt_status_count from t_rlb_jjzt t where t.jjdm =v_product_no and t.rq = to_char(to_date(p_applay_date,'yyyy-MM-dd'),'yyyyMMdd');    
            ...
            if v_pt_status_count > 0 then
                ...
                v_flag := '1';
                v_msg := '产品:' || v_product_name || ' 在日期' || p_applay_date || '不是临时开放日，不能导入订单';
            end if;
        ...
        if v_flag <> 1 then
            ...
            begin 
                 ...
                 where  bc.credit_id = v_credit_id and bc.product_id = p_product_id and bc.account_no = v_bank_accunt_no and bc.bank_no = v_bank_no and is_delete='0' and rownum=1;
            exception  when no_data_found then 
            ...
            ) values (v_table_id,v_apply_no,v_current_date,v_busi_type_code,p_product_id,v_product_no,v_rgje,v_credit_id,p_create_by,
            ...
            --对银行账户登记可放开，当五项（银行账号、银行户名、开户行名称、省份、城市）全填写时，才让其它登记银行信息
             if  ( v_bank_accunt_no_count <1 ) then  -- 不存在产品对应的银行卡
                 if  (v_bank_accunt_no is not null) and (v_bank_account_name is not null) and (v_bank_no is not null)  then                  
                      ...
                      select count(1) into v_bank_accunt_no_count from busi_bind_bank_card  t where  t.credit_id=v_credit_id and t.product_id=p_product_id  and is_delete='0' and t.is_back_account='1';                                       
                      ...
                      insert into busi_bind_bank_card(id,credit_id,user_name,bank_no,bank_name,open_bank_name,province_id,province_name,city_id,city_name,
                                 ...
                                 v_city_code, v_city_name_db,v_bank_accunt_no,p_product_id,p_company_id,v_apply_no,v_current_date,sysdate,
