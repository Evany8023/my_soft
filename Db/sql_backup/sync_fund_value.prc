create or replace procedure smzj.sync_fund_value is
  ...
  cursor fundinfo_cur is
    select t.id, to_char(t.publish_date,'yyyyMMdd') as fund_clrq, t.product_no,t.public_net_period , t.invest_net_period,  t.mgr_net_period,t.IS_CLOSE_DISPLAY_OPEN_DATE ,t.cp_id,t.name
      from busi_product t
     where t.is_validate=1
       and t.is_examine=1 
         and not exists (select id from Busi_Product_add a where t.id=a.product_id and a.fund_type='1');
   --基金信息
...
begin
  ...
  loop
    ...
    if v_fund_jztbzq = 1  then
            insert into  t_fund_value_tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)
            select TMP_SEQ.Nextval, v_fund_code,rq,v_type,v_fund_id,v_fund_value_status,v_com_id,v_fund_name from (select v_fund_clr as rq from dual);
    end if;
  ----同步成立日、开放期净值；
    if v_fund_jztbzq = 2   then
          insert into  t_fund_value_tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)   select TMP_SEQ.Nextval, v_fund_code,rq,v_type,v_fund_id,v_fund_value_status,v_com_id,v_fund_name from (
   ...
   --同步成立日、开放期、每周最后一个工作日；
    if v_fund_jztbzq = 3 then
          insert into  t_fund_value_tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)
                select TMP_SEQ.Nextval, v_fund_code,rq,v_type,v_fund_id,v_fund_value_status,v_com_id,v_fund_name from (
    ...
    if v_fund_jztbzq = 4 then
      ...
      insert into  t_fund_value_tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)
        ...
        select TMP_SEQ.Nextval,v_fund_code,rq,v_type,v_fund_id,v_fund_value_status,v_com_id,v_fund_name from (select rq  from zsta.rlb@ta_dblink where gzr='1' and rq <= v_create_date  and substr(rq, 1, 4) >= v_year);
   ...
   --每季度最后一个工作日
    if v_fund_jztbzq = 5 then
      ...
      if v_mf_code is null and  v_is_mocode_kfr_pl = 0  and v_fund_jztbzq <> 0  then
             ...
             if v_ya_day ='2' then
                 --开放期
                  insert into  t_fund_value_tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)
                   select TMP_SEQ.Nextval,v_fund_code,getpreworkday( rq),v_type,v_fund_id,0,v_com_id,v_fund_name from(
  ...
  ----同步成立日、开放期净值；
   ...
   --每季度最后一个工作日
    if v_fund_tzzjztbzq = 5 then
      ...
      if v_mf_code is null and  v_is_mocode_kfr_pl = 0 and v_fund_tzzjztbzq <> 0 then
              delete  t_fund_value_tmp t where t.fund_code= v_fund_code and t.type=v_type and  t.fund_tradedate in ( select rq from zsta.uv_xx_fundstatus@ta_dblink t where t.jjdm = v_fund_code and t.jjzt in ('0', '5', '6') and t.gzr = '1' and t.rq <= v_create_date);
             if v_ya_day ='2' then
                 --开放期
                  insert into  t_fund_value_tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)
   ...
   --每季度最后一个工作日
    ...
    if v_count >0 then
        ...
        if v_ya_day='2' then   --直销压单
          insert into    t_Fund_Value_Tmp(id,fund_code,fund_tradedate,type,fund_id,STATUS,CP_ID,fund_name)
          select TMP_SEQ.Nextval,tmp.fund_code,getPreWorkDay(tmp.fund_tradedate),tmp.type,tmp.fund_id,0,v_com_id,v_fund_name from t_Fund_Value_Tmp tmp where tmp.fund_code=v_fund_code  ;
          delete t_Fund_Value_Tmp t where t.fund_code=v_fund_code and t.status=1;
        end if;
    end if;
