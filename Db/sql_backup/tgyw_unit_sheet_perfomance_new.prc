create or replace procedure smzj.tgyw_unit_sheet_perfomance_new(p_apply_date in varchar2, --申请日期
...
---- *************************************************************************
-- SUBSYS:     托管服务
-- PROGRAM:      tgyw_unit_sheet_perfomance_new
-- RELATED TAB:
-- SUBPROG:
-- REFERENCE:
-- AUTHOR:       guxu
-- DESCRIPTION:
--                 管理人单笔录入认/申购订单(去银行账户版)
-- CREATE DATE:  2016-4-11
-- VERSION:
-- EDIT HISTORY:
-- *************************************************************************
 v_job_name varchar2(100) :='tgyw_unit_sheet_perfomance_new';
 v_msg varchar2(1000); --错误信息
 v_is_sheet char(1); --公司是否停止录单
 v_credit_id varchar2(32); --证件ID
 v_detail_id varchar2(32); --详情ID
 v_bank_card_id varchar(32) :=''; --银行卡ID设置为空
...
begin
 ...
 --新增客户
    ...
    if v_flag <> 1 then
        ...
        select (substr(to_char(sysdate,'yyyyMMddhh24mmss'),0,14) || cast(dbms_random.value(10000000,99999999) as int)) into v_apply_no from dual;
        insert into busi_sheet(id,sheet_no, sheet_create_time,business_type, pt_id,pt_no, apply_amount,credit_id, BANK_CARD_ID,DT_ID,create_by,company_id,manager_contract_status,investor_contract_status,trustee_contract_status,fund_is_receive,status, manager_fund_confirm,create_date,REMARK,is_delete,is_simple)
              values (v_sheet_id,v_apply_no,v_current_date,p_sheet_busi_type,p_pt_id,v_pt_no,p_amount,v_credit_id,v_bank_card_id, v_detail_id, p_create_by,p_cp_id, '1','1', '1','1','1','1', sysdate,p_sheet_remark,'0','1');
    end if;
...
end tgyw_unit_sheet_perfomance_new;
/
